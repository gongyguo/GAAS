...
void GAAS::reallocate(){
    d_overflow[0] = 0;
    size_t l_free, l_Total = 0;
    HANDLE_ERROR(cudaMemGetInfo(&l_free, &l_Total));
    size_t old_size = d_rr_sets.size() * sizeof(uint32_t);
    if(l_free > old_size){
        RRset_mem = 2 * old_size;
    }
    else{
        RRset_mem = old_size + l_free;
    }
    size_t max_size = static_cast<size_t>(16)*1024*1024*1024;
    if (RRset_mem > max_size){
        RRset_mem = max_size;
    }
    if(l_free < RRset_mem){
        std::cout << "Insufficient GPU memory to resize mrr, please enlarge mrr initially." <<std::endl;
        exit(0);
    }
    d_rr_sets.resize(RRset_mem/sizeof(uint32_t), 0);
    double new_size = static_cast<double>(d_rr_sets.size() * sizeof(uint32_t)) / 1024.0 / 1024.0 / 1024.0;
    std::cout << "Reallocate mRR-set " <<  new_size << " GB" <<std::endl;
...
GAAS::GAAS(std::string dataset, const double epsilon, const bool reversed,  const bool large, const bool ICLT) : epsilon{epsilon}
{
    ...
    size_t l_Total = 0;
    HANDLE_ERROR(cudaMemGetInfo(&l_free, &l_Total));
    if (l_free > static_cast<size_t>(16)*1024*1024*1024){// limit by uint32 indexing
        RRset_mem = static_cast<size_t> (16)*1024*1024; //estimiate memory prior to allocation
    }
    else{
        RRset_mem = 0.95 * l_free;
    }
    d_rr_sets = thrust::device_vector<uint32_t>((RRset_mem/sizeof(uint32_t)), 0);
    double mrr_size = static_cast<double>(d_rr_sets.size() * sizeof(uint32_t)) / 1024.0 / 1024.0 / 1024.0;
    std::cout << "mRRset allocated " << mrr_size<< " GB" <<std::endl;
    length = static_cast<uint32_t>(RRset_mem / (sizeof(uint32_t))/N_BLOCKS);
    thrust::sequence(thrust::device, d_rr_block_end.begin(), d_rr_block_end.end()); 
    thrust::transform(thrust::device, d_rr_block_end.begin(), d_rr_block_end.end(), d_rr_block_end.begin(),
                    MultiplyStep(length)); 
...
{			
    ...
    if(arg.reuse)
    {
        if (n_exist_rr > 0){
            ...
            if(d_overflow[0] == 1){
                reallocate();
                gen_mrr_sets(0, n_exist_rr, arg.large);
            }
        ...
        while (sample < theta_max)
        {
            ...
            if(d_overflow[0] == 1){
                reallocate();
                gen_mrr_sets(0, sample, arg.large);
            }
